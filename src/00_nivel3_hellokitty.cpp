...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(800, 600), "Hello Kitty - Nivel 3");
    ...
    // Cargar la imagen de hellokitty amigos para mostrar después
    sf::Texture amigosTexture;
    if (!amigosTexture.loadFromFile("assets/images/hellokitty amigos.png"))
    {
        return -1;
    }
    ...
    float alturaSuelo = 635; // Altura donde está el suelo (ajustada)
    bool enElSuelo = true;
    ...
    textoWiiii.setFillColor(sf::Color::Black); // Negro
    ...
    derrotaSprite.setScale(0.08f, 0.08f); // Mismo tamaño pequeño que en nivel 2
    ...
    while (window.isOpen()) {
        ...
        // Dibujar Hello Kitty solo si no ha ganado y no ha perdido
        if (!juegoGanado && !juegoPerdido) {
            window.draw(sprite);
        }
